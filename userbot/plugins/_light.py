    ...
    if not event.text[0].isalpha() and event.text[0] not in ("/", "#", "@", "!"):
        ...
        if lightningusername is None or input_str == "text":
            ...
            for i in CMD_LIST:
                ...
                string += "\n"
            if len(string) > 4095:
                await borg.send_message(event.chat_id, "حاول `.help`")
                await asyncio.sleep(5)
            else:
                await event.edit(string)
        elif input_str:
            if input_str in CMD_LIST:
                string = "الاوامر الموجودة {}:\n".format(input_str)
                for i in CMD_LIST[input_str]:
                    string += "\n    " + i
                    string += "\n"
                await event.edit(string)
            else:
                await event.edit("`يتم التحقق..`")
                await asyncio.sleep(2)
                await event.edit(input_str + "  ☹️ الاضافة غير موجودة😞😞!")
        else:
            light_help_strin = """**المساعدة الخاصة بالاوامر** 😉😉 !\n اذا لاحظت اي خلل راسلنا @klthon:"""
